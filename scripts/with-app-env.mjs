...
/**
 * Put `node_modules/.bin` first on PATH so local CLIs resolve the same way
 * `npm run` does. Windows also needs `Path` (the legacy name) kept in sync.
 */
export function withLocalBinPath(processEnv, root = projectRoot()) {
  const env = { ...processEnv };
  const binDir = join(root, "node_modules", ".bin");
  const sep = process.platform === "win32" ? ";" : ":";
  const current = env.PATH || env.Path || "";
  const next = current.split(sep).includes(binDir) ? current : `${binDir}${sep}${current}`;
  env.PATH = next;
...
/**
 * Resolve a wrapper command to a spawn target.
 *
 * Bare `spawn("vite")` fails with ENOENT on Windows even after `npm install`:
 * npm puts `node_modules/.bin` on PATH, but `vite` there is a `.cmd` shim and
 * `child_process.spawn` without `shell` does not honor PATHEXT. Running the
 * JS entry with `process.execPath` works on every platform.
 */
export function resolveSpawnTarget(command, args, root = projectRoot()) {
  if (command === process.execPath || command.endsWith(".js") || command.endsWith(".mjs") || command.endsWith(".cjs")) {
  ...
  const local = join(root, "node_modules", ".bin", win ? `${command}.cmd` : command);
  if (existsSync(local)) {
    return { file: local, argv: args, shell: win };
  }
  return { file: command, argv: args, shell: win };
}
...
function main(argv) {
  ...
  const child = spawn(target.file, target.argv, { stdio: "inherit", env, shell: target.shell });
  ...
  child.on("error", (err) => {
    const hint =
      err?.code === "ENOENT"
        ? ` '${command}' was not found. Install packages first, then retry.`
        : "";
    console.error(`[with-app-env] failed to run ${command}:`, err?.message || err, hint);
    process.exit(127);
  });
