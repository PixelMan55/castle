...
test("vite is spawned through node and the JS entry, not a bare PATH lookup", () => {
  const target = resolveSpawnTarget("vite", ["dev", "--host", "0.0.0.0", "--port", "8080"]);
  assert.equal(target.file, process.execPath);
  assert.equal(target.argv[0], join(projectRoot(), "node_modules", "vite", "bin", "vite.js"));
  assert.deepEqual(target.argv.slice(1), ["dev", "--host", "0.0.0.0", "--port", "8080"]);
  assert.equal(target.shell, false);
});

test("a direct node invocation is left unchanged", () => {
  const target = resolveSpawnTarget(process.execPath, ["-e", "0"]);
  assert.equal(target.file, process.execPath);
  assert.deepEqual(target.argv, ["-e", "0"]);
  assert.equal(target.shell, false);
});

test("local bin dir is prepended to PATH", () => {
  const env = withLocalBinPath({ PATH: "/usr/bin" });
  const binDir = join(projectRoot(), "node_modules", ".bin");
  assert.equal(env.PATH.startsWith(`${binDir}:`) || env.PATH.startsWith(`${binDir};`), true);
});
